/*
 ...
 All the important parameters are set through this class.
...
// !! Ensure that you are using an English keyboard, the library doesn't recognize other.
...
final Vector2D step_extent = new Vector2D(40, 30); // ## {mm, mm}
final float vrt_offset = - 30.0; // ## mm
final float hrz_offset = + 0.00; // ## mm

final float min_extent = 0.001; // ## mm

/// Serial Class
final String com_port = "COM6"; // !! Replace with the port you are using
...
float turn = 0; //> indicates the direction of rotation(yaw)
float period = 4; //> indicates the kinematics class iterations every second
...
//: callback function used to detect user input
void keyPressed() {
  if (keyCode  == UP) {
  ...
  println("x:" + direction.x() + " y:" + direction.y() + " turn:" + turn); //> prints the new values of direction & turn
}
...
void init_kinematics() {
  ...
  int e = kinematics.init(body_transform, p_joint_origin, step_extent, min_extent, bone_length, frequency); //>  transfers parameters to kinematics class
  //: checks whether the object initialization was successful
